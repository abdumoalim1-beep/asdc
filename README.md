...
src/asdc/
  ...
  server.py       FastAPI فوق نفس pipeline (classify -> validate -> execute)
                  لواجهة الويب: GET /api/state، POST /api/chat، POST /api/templates/upload
web/index.html    واجهة محادثة (RTL) متصلة بالـ API الحقيقي فعليًا — الملفات
                  والإحصاءات تُقرأ من البيانات الحية، ما فيه ردود مسرحية
...
## واجهة الويب (المحادثة + الملفات بالشريط الجانبي)
...
افتح `http://127.0.0.1:8000` — الإحصاءات (عدد العملاء/المنتجات/المؤثرين/المستندات)
والملفات بالشريط الجانبي كلها من `GET /api/state` الحقيقي، وكل رسالة بالمحادثة تمر
فعليًا عبر `POST /api/chat` (نفس pipeline اللي يستخدمه `cli.py`: classify -> validate
-> execute) — ما فيه ردود مبرمجة سلفًا. لو صار خطأ تحقق (مثال: قالب غير موجود) أو
رد غير صالح من النموذج، الواجهة تعرضه بوضوح (شارة "تم الرفض" أو "خطأ") بدل ما تتظاهر
إن كل شي نجح.

### النية السابعة: `chat`

النص الأصلي للـ system prompt كان مصمم كمصنّف أوامر ضيق فقط (6 أنواع)، وما كان فيه
مكان لأسئلة عامة زي "اشرح لي مساحة العمل" — كانت تُحشر في `query` أو `ambiguous`
وتطلع ردود جافة. أضفنا نية سابعة `chat` مخصصة للترحيب والأسئلة عن النظام نفسه، تسمح
برد أطول وأدفأ لكنها **ما تلمس أي بيانات إطلاقًا** (نفس قاعدة عدم الاختراع تنطبق:
أي رقم أو اسم يذكره لازم يكون موجود حرفيًا بالسياق المُعطى). validator.py و
executor.py يعاملونها كـ no-op بالضبط زي `ambiguous`/`unsupported`.
...
ببعض في اختبار end-to-end. اختبارات `test_server.py` تشتغل بنفس الطريقة على
FastAPI مباشرة عبر `TestClient`، مع نسخة مؤقتة من `data/` حتى ما تلمس الملفات
الحقيقية بالمستودع.
